...
if app_mode=='Home':
    ...
    st.image('image\loan.jpg')
    st.markdown('Datasets : ')
    data=pd.read_csv('archive\loan_data_set.csv')
    data['LoanAmount'] = data['LoanAmount'].replace(np.nan, int(data['LoanAmount'].mean()))
    st.write(data.head())
    st.markdown('Applicant Income VS Loan Amount')
    st.subheader("ApplicantIncome and LoanAmount")
    st.bar_chart(data[['ApplicantIncome','LoanAmount']].head(20))

elif app_mode == 'Prediction':
    st.image('image\predict.jpg')
    ...
    if st.button('Predict'):
        ...
        loaded_model = pickle.load(open(r'Random_Forest.sav', 'rb'))
